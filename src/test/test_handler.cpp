...
{
  controller_request("/timers?requesting-node=10.0.0.1:9999;sync-mode=SCALE;cluster-id=cluster-id", htp_method_GET, "", "requesting-node=10.0.0.1:9999;sync-mode=SCALE;cluster-id=cluster-id");
  EXPECT_CALL(*_th, get_timers_for_node("10.0.0.1:9999", 0, "cluster-id", _)).WillOnce(Return(200));
  EXPECT_CALL(*_httpstack, send_reply(_, 200, _));
  _task->run();
}
...
TEST_F(TestHandler, ValidTimerGetLeavingNode)
{
  ...
  controller_request("/timers?requesting-node=10.0.0.4:9999;sync-mode=SCALE;cluster-id=cluster-id", htp_method_GET, "", "requesting-node=10.0.0.4:9999;sync-mode=SCALE;cluster-id=cluster-id");
...
{
  controller_request("/timers?sync-mode=SCALE;cluster-id=cluster-id", htp_method_GET, "", "sync-mode=SCALE;cluster-id=cluster-id");
  EXPECT_CALL(*_httpstack, send_reply(_, 400, _));
  _task->run();
}
...
{
  controller_request("/timers?requesting-node=10.0.0.1:9999;cluster-id=cluster-id", htp_method_GET, "", "requesting-node=10.0.0.1:9999;cluster-id=cluster-id");
  EXPECT_CALL(*_httpstack, send_reply(_, 400, _));
  _task->run();
}

// Invalid request: Tests that get requests for timer references with a
// missing cluster-id parameter gets rejected
...
{
  controller_request("/timers?requesting-node=10.0.0.5:9999;sync-mode=SCALE;cluster-id=cluster-id", htp_method_GET, "", "requesting-node=10.0.0.5:9999;sync-mode=SCALE;cluster-id=cluster-id");
  EXPECT_CALL(*_httpstack, send_reply(_, 400, _));
  _task->run();
}
...
{
  controller_request("/timers?requesting-node=10.0.0.1:9999;sync-mode=NOTSCALE;cluster-id=cluster-id", htp_method_GET, "", "requesting-node=10.0.0.1:9999;sync-mode=NOTSCALE;cluster-id=cluster-id");
  EXPECT_CALL(*_httpstack, send_reply(_, 400, _));
  _task->run();
}
